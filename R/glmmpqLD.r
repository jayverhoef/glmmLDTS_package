glmmpqLD <- function(fixed.formula,
  random.formula,
  data,
  timecol,
  EstMeth = "REML",
  ...
  lambda = 0)
{
  ...
  # fixed effects design matrix
	X0 <- model.matrix(fixed.formula, data = data)
	...
	for(i in 1:nXc) ind.not0s[i] <- (sum(abs(X0[,i])) != 0)
	X0 <- X0[,ind.not0s]
  ...
  grp.int <- as.integer(grp.vec)
	...
	#standardize the columns in X for numerical stability
	if(nXc > 1) X <- stdX(X0)
	else X <- X0
	X <- matrix(X, ncol = nXc)
	# rank of X
  ...
  psi.new <- c(psi.hat)
    ...
    # create an indicator to stop looping
      ...
      psi.est <- optimize(m2LL.1D, pos = i, interval = c(-19, 19),
          ...
          grp.vec = grp.vec, Del.i = Del.i, A.5 = A.5,
...
#				(iter >= 20)) ) stoploop <- 1
    ...
    # unstandardize beta hat and return new beta.hat and covb
	  ...
	  if(nXc > 1) {
		  beta.list <- unstdX.beta.hat(X0, beta.hat, covb)
		  beta.hat <- beta.list$beta.hat
		  covb <- beta.list$covb
		  bhat.se <- sqrt(diag(covb))
	  }
    ...
    # create fixed effects table
	  fixed.eff.est <- data.frame(parameters = beta.hat,
			...
			prob.t = round(100000*(1- pt(abs(beta.hat/bhat.se), df = n - dX))*2)/100000
		)
